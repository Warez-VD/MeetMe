...
{
    [TestFixture]
    public class RemoveNotificationStatistic_Should
    {
        [Test]
        ...
        [Test]
        public void DecreasedStatistic_NotificationCountWithOne()
        {
            // Arrange
            var mockedUserService = new Mock<IUserService>();
            var user = new CustomUser() { Id = 1 };
            mockedUserService.Setup(x => x.GetByIndentityId(It.IsAny<string>())).Returns(user);
            var mockedStatisticRepository = new Mock<IEFRepository<Statistic>>();
            var statistic = new Statistic() { CustomUserId = 1, NotificationsCount = 1 };
            ...
            // Assert
            Assert.AreEqual(statistic.NotificationsCount, 0);
        }
